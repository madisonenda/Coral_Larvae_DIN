...
### Let's start playing around with some models! :)

```{r}
acropora_lm <- lm(Adult_Pop ~ Larval_Recruits + Algae, acro_data)
summary(acropora_lm)
```
...
### Create a New Linear Regression with the 7 Year Data

```{r} 
acro_7_lm <- lm(Coral_Cover~ Larval_Recruits + Algae + Nitrogen + Temp + Larval_Recruits:Nitrogen, acro_data_7)
summary(acro_7_lm)
```
